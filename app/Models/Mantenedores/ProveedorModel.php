...
class ProveedorModel
{
	...
	{   
		...
		$stmt = $dbh->prepare('CALL sp_proveedor_listar(?, ?, ?)');
	...
	{		       
		$parametros = array(
			array_key_exists('proveedorid',$datos)? $datos['proveedorid']:NULL,			
			$datos['razonsocial'],
			$datos['telefono'],
			$datos['direccion'],
			$datos['ruc']           					
		);
		$res = DB::select("CALL sp_proveedor_ins_upd(?,?,?,?,?)",$parametros);
		return $res[0] ?? null;
	}
	...
	{		       
		...
		$res = DB::select("CALL sp_proveedor_anular(?,?)",$parametros);
		return $res[0] ?? null;
	}
